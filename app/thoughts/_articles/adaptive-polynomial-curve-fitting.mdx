...
summary: "A simple way to find a good polynomial regression curve and avoid overfitting."
draft: true
---
...
![Old analytics chart](analytics-old.png "Our old analytics visualization.")
...
![New analytics chart](analytics-new.png "Improved chart with the same data.")
...
<div style={{ "--frame-background": "#6b6c69" }}>

![Old analytics chart with first and last data points highlighted](analytics-old-2.png)
...
$$
\operatorname{MSE} = \sum{\frac{(\operatorname{Value} - \operatorname{PredictedValue})^2}{\operatorname{DataCount}}}
$$
...
You can play with the example below: it generates fake data points with some normal distributed randomness and then calculates the regression curve for it. For the “Constant” data set, it’s better to just use a straight line to fit ($\operatorname{order} = 1$). However, for the “Quadratic” data set, selecting $\operatorname{order} = 2$ will result in a better curve (and it’s stable).
...
Ideally, a good fit should have a low error for both sets, as it shouldn’t be overfitting only the training set. Hence we use a simple but intuitive equation $\operatorname{error}=max(\operatorname{MSE}(\operatorname{TrainingSet}), \operatorname{MSE}(\operatorname{TestSet}))$ to measure the “overall goodness” of the fit for the entire data set. Usually, when we increase the order of the polynomial regression, the error will decrease (underfitting), hit a good fit, then increase again (overfitting).
...
![Old analytics chart](analytics-old.png)

![New analytics chart](analytics-new.png)
