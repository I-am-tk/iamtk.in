...
summary: "“You can’t connect the dots looking forward; you can only connect them looking backwards.”"
---
...
Let's begin with the _online_ version. The naive solution involves iterating through the list and counting the number of unique items within each range. This approach has a time complexity of $O(nm)$, where $n$ is the length of the list and $m$ is the number of queries. This is because for each query, you need to iterate through the list to count the number of unique items within the specified range:

<BlockSideTitle title={<>
  Let’s assume that list access is always $O(1)$ for simplicity. This algorithm is fairly straightforward: For each query, we iterate through the list and count the number of unique items within the range.
</>}>
```js
...
<BlockSideTitle title={<>
  Sorting the queries takes $O(m \log m)$ time, where $m$ is the number of queries.
</>}>
```js
const list = inputList();
...
Next, we can iterate through the sorted queries while _maintaining the count of unique numbers within the range_. For example, when transitioning from the range `[0, 3]` to `[1, 4]`, we can simply subtract the count of $\operatorname{list}_{0}$ and add the count of number $\operatorname{list}_{4}$. All the other numbers remain unchanged.
